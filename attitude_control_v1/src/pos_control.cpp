...
/**
 *  @brief Deviation calculation
 *  @param current_val, target_val
 *  @param[out] error []
 */
float error_calc(float current_val, double target_val)
{
  float error;
 ...
 *  @param p, i, d, error, delta t 
 *  @param[out] controller_output []
 */
float PID_calc(float Kp, float Ki, float Kd, float e, double del_t)
{
  float Du_p = Kp * e;
  float Du_i = Ki * (e + e_old) / 2.0 * del_t;
  float Du_d = Kd * (e - e_old) / del_t;
  
  e_old = e;
  float Output = Du_p + Du_i + Du_d;
  return Output;
}
/*======================================================================
 ...
 * 
 *=======================================================================*/
void optical_flow_Callback(const px_comm::OpticalFlow::ConstPtr& of_msg){
  float flow_vel_x;
  float flow_vel_y;
  flow_vel_x = of_msg->velocity_x;
  flow_vel_y = of_msg->velocity_y;
